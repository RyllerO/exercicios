...
class Tree {
    ...
    public void inserir(long valorInserido) {
        No novoNo = new No(); // cria um novo Nó
        novoNo.no = valorInserido; // atribui o valor recebido ao item de dados do Nó
        novoNo.dir = null;
        novoNo.esq = null;

        if (raiz == null) { //cria a raiz caso ela não exista
            raiz = novoNo;
            ...
            while (true) {
                ...
                if (valorInserido <= valorAtual.no) { // ir para esquerda
                    valorAtual = valorAtual.esq;
                    if (valorAtual == null) {
                        valorAnterior.esq = novoNo;
                        return;
                    }
    ...
    public No buscar(long chave) {
        ...
        while (atual.no != chave) { // enquanto nao encontrou
            if (chave < atual.no) atual = atual.esq; // caminha para esquerda
    ...
    public boolean remover(long v) {
        ...
        // ****** Buscando o valor **********
        while (atual.no != v) { // enquanto nao encontrou
            pai = atual;
            if (v < atual.no) { // caminha para esquerda
                atual = atual.esq;
